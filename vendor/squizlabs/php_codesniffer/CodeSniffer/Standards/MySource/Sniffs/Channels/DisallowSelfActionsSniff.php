<?php
/**
 * Ensures that self and staticps are not used to call public methods in action classes.
 *
 * PHP version 5
 *
 * @category  PHP
...
class MySource_Sniffs_Channels_DisallowSelfActionsSniff implements PHP_CodeSniffer_Sniff
{
    ...
    public function process(PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
        ...
        // Find all staticps method calls in the form self::method() in the class.
        ...
        for ($i = ($classNameToken + 1); $i < $classEnd; $i++) {
            ...
            if ($tokens[$prevToken]['content'] !== 'self'
                && $tokens[$prevToken]['content'] !== 'staticps'
            ) {
                continue;
            }
